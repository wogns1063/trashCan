...
#### 전력 계산
- USB로 연결하는 아두이노는 약 500mA를 공급받고 5V핀으로 **450mA**까지 쓸 수 있다.
- 위의 회로도에서 서보모터는 동시에 작동하지 않기 때문에 **전력소비가 큰 서보모터 기준으로 계산**하였다. 
  (나머지 모터는 유휴전력으로 계산)
- Opnner 모터는 작동 전압 6v, 작동 전류 300~350mA, Bottom 모터는 작동 전압 5V, 유휴 전류 7.5mA이다.
- 총 작동 전류는 307.5mA ~ 357.5mA이다.
...
#### 전력 계산
- 5.1v 마이크로 USB로 전원을 공급받은 라즈베리파이는 **500mA**까지 쓸 수 있다.
- 위의 회로도에서 동시에 작동하는 서보모터는 2개(GPIO 16)이고, 나머지는 유휴 전력으로 계산하였다.
- Grip 모터(GPIO 16)는 작동 전압 5v, 작동 전류 400~500mA, 나머지 서보모터는 유휴 전류 각 7.5mA이다.
- 총 작동 전류는 407.5mA ~ 507.5mA이다.
<br>
...
Ir = 23 
GPIO.setup(Ir, GPIO.IN)  #IR SENSOR Vout
            ...
            def run(self):
                while True:
                    if self.__exit:
                        ...
                        val = GPIO.input(Ir)    #IRsensor value       
                        throwawayLid(val)
...
<img src="http://www.devicemart.co.kr/data/collect_img/kind_0/goods/large/38487.jpg" height="70px">  1. 디지털 적외선 모션 센서 [SEN0018] - 물체 동작 감지
...
<img src="http://www.devicemart.co.kr/data/collect_img/kind_0/goods/large/1265392.jpg" height="80px">  3. 최고급형 메탈 서보모터-DT2100 - 뚜껑 열기

<img src="http://robomecha.co.kr/web/product/big/robomecha_49.jpg" height="80px">  4. Servo Motor (MTS-A410SE) - 3번 모터 제어

<img src="http://www.devicemart.co.kr/data/collect_img/kind_0/goods/large/200803051448440.jpg" height="80px">  5. 서보모터(HS-311) - 그리퍼 제어, 그리퍼 회전, 수거함 회전

<img src="http://www.devicemart.co.kr/data/collect_img/kind_0/fogoods/large/1278810.jpg" height="80px">  6. Standard Gripper Kit A (637094) [ROB-13174] - 컵 고정

<img src="https://user-images.githubusercontent.com/41332126/70533270-19356800-1b9c-11ea-89f4-d5958f5ed757.png" height="80px">  7. NTS0090 아날로그 서보모터 - 그리퍼 제어

<img src="http://image.auction.co.kr/itemimage/12/0c/27/120c278486.jpg" height="80px">  8. 미니 랜턴 - 컵에 빛 투과
...
## 6. 동작
[![전체동작](http://img.youtube.com/vi/vKi94sMjuI0/0.jpg)](https://youtu.be/vKi94sMjuI0) ← (Click!)전체 동작
